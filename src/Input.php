...
class Input
{
    ...
    public function __construct($version = 0, $level = Str::QR_ECLEVEL_L)
    {
        if ($version < 0 || $version > Spec::QRSPEC_VERSION_MAX || $level > Str::QR_ECLEVEL_H) {
    ...
    public function insertStructuredAppendHeader($size, $index, $parity)
    {
        ...
        if ($index <= 0 || $index > static::MAX_STRUCTURED_SYMBOLS) {
    ...
    public function calcParity()
    {
        ...
        foreach ($this->items as $item) {
            if ($item->mode != Str::QR_MODE_STRUCTURE) {
